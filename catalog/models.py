...
class GIRAttribute(Attribute):
    """An attribute indicating that a course satisfies a GIR requirement."""
    attributes = [
        "REST",
        "LAB2",
        "LAB",
        ...
        "PHY1",
        "PHY2"]
    # most GIR attributes should only be once (e.g. CAL1), but some can be
    # used twice (e.g. REST) - changed in __init__
    multiple = False
    ...
    def modify_course(self, course):
        """Adds this attribute to the course."""
        course.gir_attribute = self.requirement
        return course


class HASSAttribute(Attribute):
    """An attribute indicating that a course satisfies a HASS requirement."""
...
class CommunicationAttribute(Attribute):
    """An attribute indicating that a course satisfies a CI requirement."""
...
class CourseFields:
    """Fields used to populate the JSON returned by the catalog API."""
...
CSV_HEADERS = {
    ...
    CourseAttribute.equivalentSubjects:
        (CourseFields.equivalent_subjects, list_converter),
    CourseAttribute.jointSubjects:
        (CourseFields.joint_subjects, list_converter),
    CourseAttribute.meetsWithSubjects:
        (CourseFields.meets_with_subjects, list_converter),
    ...
    CourseAttribute.communicationRequirement:
        (CourseFields.communication_requirement, string_converter),
    CourseAttribute.hassRequirement:
        (CourseFields.hass_attribute, string_converter),
    ...
    CourseAttribute.offeredSpring:
        (CourseFields.offered_spring, bool_converter),
    CourseAttribute.offeredSummer:
        (CourseFields.offered_summer, bool_converter),
    CourseAttribute.totalUnits: (CourseFields.total_units, int_converter),
    CourseAttribute.isVariableUnits:
        (CourseFields.is_variable_units, bool_converter),
    ...
    CourseAttribute.preparationUnits:
        (CourseFields.preparation_units, int_converter),
    ...
    CourseAttribute.notOfferedYear:
        (CourseFields.not_offered_year, string_converter),
    CourseAttribute.quarterInformation:
        (CourseFields.quarter_information, string_converter),
    ...
    CourseAttribute.averageInClassHours:
        (CourseFields.in_class_hours, float_converter),
    CourseAttribute.averageOutOfClassHours:
        (CourseFields.out_of_class_hours, float_converter),
    CourseAttribute.enrollment:
        (CourseFields.enrollment_number, float_converter),
    CourseAttribute.eitherPrereqOrCoreq:
        (CourseFields.either_prereq_or_coreq, bool_converter),
    CourseAttribute.sourceSemester:
        (CourseFields.source_semester, string_converter),
...
FIELD_TO_CSV = {field_name: csv_header for csv_header,
                (field_name, _) in CSV_HEADERS.items()}


class Course(models.Model):
    """A Django model that represents an MIT course. Courses can be public,
    corresponding to the courses loaded from the catalog, or private
    (corresponding to user-created courses)."""
    ...
    def public_courses(cls):
        """Returns a QuerySet containing all public courses."""
    ...
    def make_generic(cls, subject_id, unique_id):
        ...
        spaces in the given subject ID, e.g. "CI-H HASS-A". Returns a Course
        whose subject ID has unique ID appended. Raises a ValueError if one or
        more generic attributes are invalid."""

        is_generic_course = False
        if "." not in subject_id:
            # potential a generic course
            # generic course could have more than one attribute, e.g. CI-H
            # HASS-A
            subject_ids = subject_id.split(" ")
            # dict of attributes and values to add to created Course object
            matching_attributes = []
            # attributes to test for in generic course (gets a list of
            # CourseAttributeLists properties that aren't hidden)
            tested_attributes = [
                GIRAttribute,
                HASSAttribute,
            ...
            for subject_attribute in subject_ids:
                ...
                for attribute in tested_attributes:
                    # if the course matches a generic attribute, it is a
                    # generic course with that attribute
        ...
        if is_generic_course:
            generic_course = Attribute.combine(
                matching_attributes, unique_id).course
    ...
    def get_equivalent_subjects(self):
        """Gets this course's equivalent subjects as Course objects."""
        return self._get_courses(self.equivalent_subjects)

    def get_joint_subjects(self):
        """Gets this course's joint subjects as Course objects."""
        return self._get_courses(self.joint_subjects)

    def get_meets_with_subjects(self):
        """Gets this course's meets-with subjects as Course objects."""
    ...
    def get_hass_attributes(self):
        """Returns a list of HASS attributes stored in the hass_attribute
        field."""
    ...
    def __unicode__(self):
        return u"<Course {}: {}>".format(self.subject_id, self.title)
    ...
    def satisfies(self, requirement, all_courses=None):
        ...
        if ("GIR:" in requirement and self.gir_attribute and
                self.gir_attribute == req):
            return True
        ...
        if ("CI-" in req and self.communication_requirement and
                self.communication_requirement == req):
            return True

        if (self.subject_id == req or req in self.joint_subjects.split(",") or
                req in self.equivalent_subjects.split(",")):
            return True
        ...
        if (all_courses is not None and self.parent is not None and
                req == self.parent):
            try:
                parent_course = Course.public_courses().get(
                    subject_id=self.parent)
                ids = set(c.subject_id for c in all_courses)
                if (parent_course.children is not None and
                        all(child in ids
                            for child in parent_course.children.split(","))):
                    return True
